// backend/routes/asignacionRoutes.js - VERSIÓN COMPLETA CON PDF Y PRÉSTAMOS
...
// ============================================
// FUNCIÓN COMPLETA PARA GENERAR ACTA DE ASIGNACIÓN (CON PRÉSTAMO)
// ============================================
function generarActaAsignacion(datos) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Título según tipo
            const titulo = es_prestamo ? 'ACTA DE PRÉSTAMO DE EQUIPOS' : 'ACTA DE ENTREGA DE EQUIPOS';
            doc.font('Helvetica-Bold').fontSize(16).text(titulo, { align: 'center' }).moveDown(0.5);
            doc.font('Helvetica').fontSize(10)
               .text(`Fecha: ${formatearFecha(fecha_asignacion)}`, { align: 'left' })
               .text(`ID ${es_prestamo ? 'Préstamo' : 'Asignación'}: ${id_asignacion}`, { align: 'left' })
               .moveDown(1);
            ...
            doc.font('Helvetica-Bold').fontSize(12).text('4. MOTIVO', { underline: true }).moveDown(0.5);
            doc.font('Helvetica').fontSize(10).text(motivo || `${es_prestamo ? 'Préstamo' : 'Asignación'} de equipo`, { width: 500 }).moveDown(2);
            ...
            // 6. CONDICIONES ESPECIALES PARA PRÉSTAMO
            if (es_prestamo) {
                doc.font('Helvetica-Bold').fontSize(12).text('6. CONDICIONES DEL PRÉSTAMO', { underline: true }).moveDown(0.5);
                doc.font('Helvetica').fontSize(10)
                   .text('• El equipo debe ser devuelto en las mismas condiciones en que fue entregado.', { indent: 20 })
                   .text('• Cualquier daño o pérdida será responsabilidad del trabajador.', { indent: 20 })
                   .text('• La devolución deberá realizarse en la fecha acordada.', { indent: 20 });
                doc.moveDown(1);
            }

            // ========== SEGUNDA HOJA - FIRMAS ==========
            doc.addPage();
            doc.font('Helvetica-Bold').fontSize(14).text('FIRMAS', { align: 'center', underline: true }).moveDown(3);

            // Firma Trabajador
            doc.font('Helvetica-Bold').fontSize(11).text('RECIBÍ CONFORME', { align: 'center' }).moveDown(1);
            const lineaTrabajadorY = doc.y;
            doc.moveTo(120, lineaTrabajadorY).lineTo(480, lineaTrabajadorY).stroke();
            dibujarFirma(doc, firma_trabajador, 120, lineaTrabajadorY - 25, colaborador.nombre);
            doc.moveDown(2);
            doc.font('Helvetica').fontSize(9).text('FIRMA TRABAJADOR', { align: 'center' }).moveDown(3);

            // Firma Gerente
            doc.font('Helvetica-Bold').fontSize(11).text('ENTREGÓ CONFORME', { align: 'center' }).moveDown(1);
            const lineaGerenteY = doc.y;
            doc.moveTo(120, lineaGerenteY).lineTo(480, lineaGerenteY).stroke();
            dibujarFirma(doc, firma_gerente, 120, lineaGerenteY - 25, EMPRESA.representante_legal);
            doc.moveDown(2);
            doc.font('Helvetica').fontSize(9).text(EMPRESA.cargo_representante, { align: 'center' }).moveDown(2);

            // Nota al pie
            doc.font('Helvetica-Oblique').fontSize(8)
               .text(`Este documento es una representación digital de la ${es_prestamo ? 'préstamo' : 'entrega'} de equipos.`, { align: 'center' })
...
// ============================================
// FUNCIÓN COMPLETA PARA GENERAR ACTA DE RECEPCIÓN (CON PRÉSTAMO)
// ============================================
function generarActaRecepcion(datos) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const titulo = es_prestamo ? 'ACTA DE DEVOLUCIÓN DE PRÉSTAMO' : 'ACTA DE RECEPCIÓN DE EQUIPOS';
            doc.font('Helvetica-Bold').fontSize(16).text(titulo, { align: 'center' }).moveDown(0.5);
            doc.font('Helvetica').fontSize(10)
            ...
            doc.font('Helvetica').fontSize(10).text(motivo || 'Devolución de equipo', { width: 500 }).moveDown(2);
            ...
            // Firma Trabajador (quien devuelve)
            const textoFirmaTrabajador = es_prestamo ? 'DEVUELVO CONFORME' : 'ENTREGÓ CONFORME';
            doc.font('Helvetica-Bold').fontSize(11).text(textoFirmaTrabajador, { align: 'center' }).moveDown(1);
            const lineaTrabajadorY = doc.y;
            doc.moveTo(120, lineaTrabajadorY).lineTo(480, lineaTrabajadorY).stroke();
            dibujarFirma(doc, firma_trabajador, 120, lineaTrabajadorY - 25, colaborador.nombre);
            doc.moveDown(2);
            ...
            // Firma Gerente (quien recibe)
            const textoFirmaGerente = es_prestamo ? 'RECIBÍ CONFORME' : 'RECIBÍ CONFORME';
            doc.font('Helvetica-Bold').fontSize(11).text(textoFirmaGerente, { align: 'center' }).moveDown(1);
            const lineaGerenteY = doc.y;
            doc.moveTo(120, lineaGerenteY).lineTo(480, lineaGerenteY).stroke();
            dibujarFirma(doc, firma_gerente, 120, lineaGerenteY - 25, EMPRESA.representante_legal);
            doc.moveDown(2);
            ...
            // Nota al pie
            doc.font('Helvetica-Oblique').fontSize(8)
               .text(`Este documento es una representación digital de la ${es_prestamo ? 'devolución de préstamo' : 'recepción'} de equipos.`, { align: 'center' })
               .text('Los datos contenidos en este documento son de carácter informativo.', { align: 'center' });
...
router.post('/generar-acta-asignacion', async (req, res) => {
    try {
        ...
        const tipoArchivo = es_prestamo ? 'prestamo' : 'asignacion';
        const filename = `acta_${tipoArchivo}_${id_asignacion || Date.now()}.pdf`;
        const filepath = path.join(DOCS_DIR, filename);
        fs.writeFileSync(filepath, pdfBuffer);
        
        console.log(`✅ Acta de ${es_prestamo ? 'préstamo' : 'asignación'} generada: ${filename}`);
        
        res.json({
            success: true,
            message: `Acta de ${es_prestamo ? 'préstamo' : 'asignación'} generada exitosamente`,
            filename: filename,
...
router.post('/generar-acta-recepcion', async (req, res) => {
    try {
        ...
        const tipoArchivo = es_prestamo ? 'devolucion_prestamo' : 'recepcion';
        const filename = `acta_${tipoArchivo}_${id_asignacion || Date.now()}.pdf`;
        const filepath = path.join(DOCS_DIR, filename);
        fs.writeFileSync(filepath, pdfBuffer);
        
        console.log(`✅ Acta de ${es_prestamo ? 'devolución de préstamo' : 'recepción'} generada: ${filename}`);
        
        res.json({
            success: true,
            message: `Acta de ${es_prestamo ? 'devolución de préstamo' : 'recepción'} generada exitosamente`,
            filename: filename,
            filepath: `/uploads/documentos/${filename}`
        });
...
router.put('/:asignacionId/finalizar', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // Combinar motivo y observaciones
...
[FECHA RECEPCIÓN]: ${new Date().toLocaleString()}`;
            ...
            const filename = `acta_${esPrestamo ? 'devolucion_prestamo' : 'recepcion'}_${asignacionIdNum}_${Date.now()}.pdf`;
            const filepath = path.join(DOCS_DIR, filename);
            fs.writeFileSync(filepath, pdfBuffer);
            console.log(`✅ Acta de ${esPrestamo ? 'devolución de préstamo' : 'recepción'} guardada: ${filename}`);
